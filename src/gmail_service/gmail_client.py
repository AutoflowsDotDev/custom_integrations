...
class GmailClient:
    ...
    def get_email_details(self, message_id: str) -> Optional[EmailData]:
        ...
        if not self.service:
            error_msg = "Gmail service is not initialized"
            logger.error(error_msg)
        ...
        try:
            ...
            error_msg = f'Gmail API error occurred while fetching email {message_id}: {e}'
            logger.error(error_msg)
            raise GmailAPIError(error_msg) from e
        except ValueError as e:
            error_msg = f"Value error processing email {message_id}: {e}"
            logger.error(error_msg)
            raise MessageProcessingError(error_msg) from e
        except Exception as e:
            error_msg = f"Unexpected error fetching email {message_id}: {e}"
            logger.error(error_msg)
            raise MessageProcessingError(error_msg) from e
